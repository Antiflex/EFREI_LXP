...
namespace GLTFast.Materials
{
    ...
    public class BuiltInMaterialGenerator : MaterialGenerator
    {
        ...
        public override Material GenerateMaterial(
            Schema.MaterialBase gltfMaterialBase,
            IGltfReadable gltf,
        ...
        )
        {
            ...
            var isUnlit = gltfMaterialBase.Extensions?.KHR_materials_unlit != null;

            if (gltfMaterialBase.Extensions?.KHR_materials_pbrSpecularGlossiness != null)
            {
                material = GetPbrSpecularGlossinessMaterial(gltfMaterialBase.doubleSided);
            }
            else
            if (isUnlit)
            {
                material = GetUnlitMaterial(gltfMaterialBase.doubleSided);
            }
            else
            {
                material = GetPbrMetallicRoughnessMaterial(gltfMaterialBase.doubleSided);
            }
            ...
            material.name = gltfMaterialBase.name;

            StandardShaderMode shaderMode = StandardShaderMode.Opaque;
            Color baseColorLinear = Color.white;

            if (gltfMaterialBase.GetAlphaMode() == AlphaMode.Mask)
            {
                material.SetFloat(AlphaCutoffProperty, gltfMaterialBase.alphaCutoff);
                shaderMode = StandardShaderMode.Cutout;
            }
            else if (gltfMaterialBase.GetAlphaMode() == AlphaMode.Blend)
            {
                SetAlphaModeBlend(material);
                shaderMode = StandardShaderMode.Fade;
            }

            if (gltfMaterialBase.Extensions != null)
            {
                // Specular glossiness
                Schema.PbrSpecularGlossiness specGloss = gltfMaterialBase.Extensions.KHR_materials_pbrSpecularGlossiness;
                if (specGloss != null)
                {
            ...
            if (gltfMaterialBase.PbrMetallicRoughness != null
                // If there's a specular-glossiness extension, ignore metallic-roughness
                // (according to extension specification)
                && gltfMaterialBase.Extensions?.KHR_materials_pbrSpecularGlossiness == null)
            {
                baseColorLinear = gltfMaterialBase.PbrMetallicRoughness.BaseColor;
                material.SetFloat(MetallicProperty, gltfMaterialBase.PbrMetallicRoughness.metallicFactor);
                material.SetFloat(RoughnessFactorProperty, gltfMaterialBase.PbrMetallicRoughness.roughnessFactor);

                TrySetTexture(
                    gltfMaterialBase.PbrMetallicRoughness.BaseColorTexture,
                    material,
                    gltf,
                    BaseColorTextureProperty,
                    BaseColorTextureScaleTransformProperty,
                ...
                if (TrySetTexture(
                    gltfMaterialBase.PbrMetallicRoughness.MetallicRoughnessTexture,
                    material,
                    gltf,
            ...
            if (TrySetTexture(
                gltfMaterialBase.NormalTexture,
                material,
                gltf,
            ...
            ))
            {
                ...
                material.SetFloat(NormalTextureScaleProperty, gltfMaterialBase.NormalTexture.scale);
            }

            if (TrySetTexture(
                gltfMaterialBase.OcclusionTexture,
                material,
                gltf,
                ...
                ))
            {
                ...
                material.SetFloat(OcclusionTextureStrengthProperty, gltfMaterialBase.OcclusionTexture.strength);
            }

            if (TrySetTexture(
                gltfMaterialBase.EmissiveTexture,
                material,
                gltf,
            ...
            if (gltfMaterialBase.Extensions != null)
            {
                ...
                var transmission = gltfMaterialBase.Extensions.KHR_materials_transmission;
            ...
            switch (shaderMode)
            {
                case StandardShaderMode.Cutout:
                    SetAlphaModeMask(material, gltfMaterialBase);
                    break;
            ...
            if (gltfMaterialBase.Emissive != Color.black)
            {
                material.SetColor(EmissiveFactorProperty, gltfMaterialBase.Emissive.gamma);
                material.EnableKeyword(k_EmissionKeyword);
            }
