			...
			if (root != null)
			{
				...
				if (root.transform.childCount > 0 && nextDepth <= maxDepth)
				{
					for (int i = 0; i < root.transform.childCount; i++)
					{
						...
						int currentChildIndexInHierarchy = i + currentIndexInHierarchy + 1;

						if (currentChildIndexInHierarchy == searchedIndexInHierarchy)
						{
						...
						var nextObject = GetGameObjectByIndexInTotalHierarchy(
							root.transform.GetChild(i).gameObject, searchedIndexInHierarchy, maxDepth, currentChildIndexInHierarchy, nextDepth);
