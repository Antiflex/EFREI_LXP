...
Shader "Custom/MRTKStandardModelObject"
{
	Properties
	{
		// Main maps.
		...
		SubShader
			{
				Pass
				{
					...
					#pragma multi_compile _ _CLIPPING_SPHERE2
	...
	#if defined(_CLIPPING_PLANE) || defined(_CLIPPING_SPHERE) || defined(_CLIPPING_BOX) || defined(_CLIPPING_PLANE2) || defined(_CLIPPING_SPHERE2) || defined(_CLIPPING_BOX2)
			...
			#undef _CLIPPING_PRIMITIVE
				...
				UNITY_DEFINE_INSTANCED_PROP(fixed, _ClipPlaneSide2)
				UNITY_DEFINE_INSTANCED_PROP(float4, _ClipPlane2)
	#endif

	#if defined(_CLIPPING_SPHERE2)
				UNITY_DEFINE_INSTANCED_PROP(fixed, _ClipSphereSide2)
	...
	#define PROXIMITY_LIGHT_DATA_SIZE 6
				...
				// Texturing.
					...
					_Metallic = channel.b;
					albedo.rgb *= channel.r;
					_Smoothness = 1.0 - channel.g;
	#else
	...
	#endif
#if defined(_CLIPPING_PLANE2)
					fixed clipPlaneSide2 = UNITY_ACCESS_INSTANCED_PROP(Props, _ClipPlaneSide2);
					float4 clipPlane2 = UNITY_ACCESS_INSTANCED_PROP(Props, _ClipPlane2);
					primitiveDistance = min(primitiveDistance, PointVsPlane(i.worldPosition.xyz, clipPlane2) * clipPlaneSide2);
#endif
#if defined(_CLIPPING_SPHERE2)
					fixed clipSphereSide2 = UNITY_ACCESS_INSTANCED_PROP(Props, _ClipSphereSide2);
					float4x4 clipSphereInverseTransform2 = UNITY_ACCESS_INSTANCED_PROP(Props, _ClipSphereInverseTransform2);
					primitiveDistance = min(primitiveDistance, PointVsSphere(i.worldPosition.xyz, clipSphereInverseTransform2) * clipSphereSide2);
#endif
#if defined(_CLIPPING_BOX2)
	...
	#if defined(_EMISSION)
	#if defined(_CHANNEL_MAP)
					output.rgb += _EmissiveColor * channel.a;
	#else
